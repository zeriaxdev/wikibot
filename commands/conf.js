...
exports.run = async (client, message, [action, key, ...value], level) => {
  ...
  // Changing the default value of a key only modified it for guilds that did not change it to another value.
    ...
    settings.set("default", defaults);
    message.reply({
      content: `✍️ \`${key}\` was successfully edited to \`${value.join(
        " "
      )}\`!`,
      allowedMentions: { repliedUser: replying === "true" },
    });
  ...
  // MAKE SURE THAT KEY IS REALLY NO LONGER NEEDED!
    ...
    const response = await awaitReply(
      message,
      `⚠️ Are you sure you want to permanently delete \`${key}\` from all guilds? This **CANNOT** be undone.`
    );
    ...
    if (["y", "yes"].includes(response)) {
      ...
      message.reply({
        content: `🗑️ \`${key}\` was successfully deleted.`,
        allowedMentions: { repliedUser: replying === "true" },
      });
    }
